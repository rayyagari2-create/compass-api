...
# -----------------------------
def make_trace(decision: str, reason: str, agent: str, capability: str, result: str, confidence: str = "high") -> Dict[str, Any]:
    return {
...
def orchestrate(req: OrchestrateRequest):
    ...
    if pending and pending.get("type") == "transfer" and pending.get("stage") == "awaiting_amount":
        ...
        if amt is None:
            ...
            return {
                ...
                "debug": {"intent": "bank_transfer", "entities": {}, "policy": {"allow": True, "risk": "medium", "reason": "Needs amount"}, "agent_trace": trace, "ts": now_ts()},
            }
        ...
        if not policy["allow"]:
            ...
            return {
                ...
                "debug": {"intent": "bank_transfer", "entities": entities, "policy": policy, "agent_trace": trace, "ts": now_ts()},
            }
    ...
    if intent == "bank_insights":
        trace = make_trace(
            ...
            agent="BankingAgent",
            capability="Insights",
            result="Returned insight summary card",
            confidence="high",
        )
        return {
            "session_id": req.session_id,
            ...
            "debug": {"intent": intent, "entities": {}, "policy": {"allow": True, "risk": "low", "reason": "Allowed"}, "agent_trace": trace, "ts": now_ts()},
        }
    ...
    if intent == "assets_cd_maturity":
        trace = make_trace(
            ...
            result="Returned CD maturity card",
            confidence="high",
        )
        return {
            "session_id": req.session_id,
    ...
    if intent == "travel_upcoming":
        trace = make_trace(
            ...
            result="Returned upcoming travel card",
            confidence="high",
        )
        return {
            "session_id": req.session_id,
    ...
    if intent == "bank_account_summary":
        ...
        return {
            ...
            "card": {"title": "Account Summary", "subtitle": "Balances overview (demo)", "body": f"Checking: ${b['checking']:.2f}\nSavings: ${b['savings']:.2f}", "actions": []},
            "debug": {"intent": intent, "entities": {}, "policy": {"allow": True, "risk": "low", "reason": "Allowed"}, "agent_trace": trace, "ts": now_ts()},
        }
    ...
    if intent == "bank_spend_analysis":
        ...
        return {
            ...
            "debug": {"intent": intent, "entities": {}, "policy": {"allow": True, "risk": "low", "reason": "Allowed"}, "charts": charts, "agent_trace": trace, "ts": now_ts()},
        }
    ...
    if intent == "bank_transfer":
        ...
        if (from_acct is None or to_acct is None) and amt is None:
            ...
            return {
                ...
                "debug": {"intent": intent, "entities": {}, "policy": {"allow": True, "risk": "medium", "reason": "Needs direction"}, "agent_trace": trace, "ts": now_ts()},
            }
        ...
        if (from_acct is None or to_acct is None) and amt is not None:
            ...
            return {
                ...
                "card": card_transfer_choose_direction(),
                "debug": {"intent": intent, "entities": {"amount": float(amt)}, "policy": {"allow": True, "risk": "medium", "reason": "Needs direction"}, "agent_trace": trace, "ts": now_ts()},
            }
        ...
        if from_acct and to_acct and amt is None:
            ...
            return {
                ...
                "debug": {"intent": intent, "entities": {"from_account": from_acct, "to_account": to_acct}, "policy": {"allow": True, "risk": "medium", "reason": "Needs amount"}, "agent_trace": trace, "ts": now_ts()},
            }
        ...
        if not policy["allow"]:
            ...
            return {
                ...
                "card": {"title": "Transfer blocked", "subtitle": "Policy check", "body": policy["reason"], "actions": []},
                "debug": {"intent": intent, "entities": entities, "policy": policy, "agent_trace": trace, "ts": now_ts()},
            }

        action_id = str(uuid.uuid4())
        state["pending_action"] = {"id": action_id, "type": "transfer", "stage": "awaiting_confirm", "from_account": from_acct, "to_account": to_acct, "amount": float(amt)}
...
def action(req: ActionRequest):
    ...
    if req.action_name == "transfer_set_direction":
        ...
        return {"ok": True, "messages": [{"role": "assistant", "content": msg}], "card": card_transfer_ask_amount(from_acct, to_acct), "debug": {"action": req.action_name, "ts": now_ts()}}

    # Cancel transfer
    if req.action_name == "cancel_transfer":
        state["pending_action"] = None
        return {"ok": True, "messages": [{"role": "assistant", "content": "Transfer cancelled (demo)."}], "card": {"title": "Cancelled", "subtitle": "No changes made", "body": "Nothing was transferred.", "actions": []}, "debug": {"action": req.action_name, "ts": now_ts()}}
    ...
    if req.action_name == "confirm_transfer":
        ...
        if amount > float(b.get(from_acct, 0)):
            state["pending_action"] = None
            return {"ok": False, "messages": [{"role": "assistant", "content": f"Insufficient funds in {from_acct.title()} (demo)."}], "card": {"title": "Transfer blocked", "subtitle": "Insufficient funds", "body": "Not enough balance.", "actions": []}}
        ...
        return {
            ...
            "card": {"title": "Transfer complete", "subtitle": "Updated balances (demo)", "body": f"Checking: ${b['checking']:.2f}\nSavings: ${b['savings']:.2f}", "actions": []},
            "balances": b,
    ...
    if req.action_name == "agent_connect":
        ...
        return {
            "ok": True,
            "messages": [{"role": "assistant", "content": "Connected. An agent has your context (demo)."}],
